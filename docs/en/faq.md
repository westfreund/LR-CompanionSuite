...
### It says “catalog side files present”. Is that bad?

`.lrcat-wal` and `.lrcat-shm` are SQLite working files. Harmless in themselves;
open and close the catalog once in Lightroom so it flushes them.
